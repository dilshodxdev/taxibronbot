...
class OrdersHandler {
    ...
    static async showOrders(ctx, page = 1) {
    ...
    static async updateOrderStatus(ctx, orderId, newStatus) {
        const validStatuses = ['pending', 'confirmed', 'completed', 'cancelled'];
        if (!validStatuses.includes(newStatus)) {
            await ctx.reply("❗ Noto'g'ri holat. Ruxsat etilgan holatlar: pending, confirmed, completed, cancelled");
            return;
        }
        const success = this.orderStorage.updateOrderStatus(orderId, newStatus);
        if (success) {
            await ctx.reply(`✅ Buyurtma ${orderId} holati "${this.getStatusText(newStatus)}" ga yangilandi.`);
        }
        else {
    ...
    static async showOrderDetails(ctx, orderId) {
   ...
   🎯 <b>Qayerga:</b> ${order.toRegion || "—"} — ${order.toDistrict || "—"}
    `;
        const keyboard = telegraf_1.Markup.inlineKeyboard([
            [
                telegraf_1.Markup.button.callback("✅ Tasdiqlash", `status_${order.id}_confirmed`),
                telegraf_1.Markup.button.callback("✅ Bajarildi", `status_${order.id}_completed`)
            ],
            [
                telegraf_1.Markup.button.callback("❌ Bekor qilish", `status_${order.id}_cancelled`),
                telegraf_1.Markup.button.callback("🔄 Qayta ko'rish", `status_${order.id}_pending`)
            ],
            [telegraf_1.Markup.button.callback("🔙 Orqaga", "back_to_orders")]
        ]);
        await ctx.reply(message, {
            parse_mode: "HTML",
            reply_markup: keyboard.reply_markup
    ...
    static createPaginationKeyboard(currentPage, totalPages) {
        const keyboard = [];
        // Sahifa navigatsiyasi
        if (totalPages > 1) {
            const navigationRow = [];
            if (currentPage > 1) {
                navigationRow.push(telegraf_1.Markup.button.callback("⬅️ Oldingi", `page_${currentPage - 1}`));
            }
            if (currentPage < totalPages) {
                navigationRow.push(telegraf_1.Markup.button.callback("Keyingi ➡️", `page_${currentPage + 1}`));
            }
    ...
    static createOrdersKeyboard(currentPage, totalPages, orders) {
        const keyboard = [];
        // Har bir buyurtma uchun ID tugmasi
        orders.forEach(order => {
            keyboard.push([
                telegraf_1.Markup.button.callback(`📋 ${order.id}`, `order_${order.id}`)
            ]);
        });
        // Sahifa navigatsiyasi
        if (totalPages > 1) {
...
OrdersHandler.orderStorage = orderStorage_1.OrderStorageService.getInstance();
