...
function setupAdminBot(bot) {
    const ADMIN_ID = Number(config_1.CONFIG.ADMIN_ID);
    const SUPER_ADMIN_ID = Number(config_1.CONFIG.SUPER_ADMIN_ID);
    const CHANNEL_ID = config_1.CONFIG.CHANNEL_ID;
    ...
    bot.command("admin", async (ctx) => {
        ...
        await ctx.reply("👋 Xush kelibsiz, Admin!", (0, start_1.AdminStart)());
    });
    ...
    async function showOrders(ctx, page = 1) {
        try {
            ...
            result.orders.forEach((order, index) => {
                ...
                message += `   📍 ${order.fromRegion}${order.fromDistrict ? ` - ${order.fromDistrict}` : ""} → ${order.toRegion}${order.toDistrict ? ` - ${order.toDistrict}` : ""}\n`;
                message += `   📅 ${orderDate}\n`;
                message += `   ${statusEmoji} ${statusText}\n\n`;
            });
            const keyboard = createOrdersKeyboard(page, result.totalPages, result.orders);
...
📍 <b>Yo'nalish:</b> ${order.fromRegion}${order.fromDistrict ? ` - ${order.fromDistrict}` : ""} → ${order.toRegion}${order.toDistrict ? ` - ${order.toDistrict}` : ""}
...
${statusEmoji} <b>Holat:</b> ${statusText}
      `;
            ...
            await ctx.reply("❌ Buyurtma ma'lumotlarini ko'rsatishda xatolik yuz berdi.");
        }
    }
...
🔄 <b>Oxirgi yangilanish:</b> ${stats.updatedAt.toLocaleString("uz-UZ", {
    ...
    // Batafsil statistika
    async function showDetailedStatistics(ctx) {
        try {
            const stats = await databaseService.getBotStatistics();
            if (!stats) {
                await ctx.reply("❌ Statistika ma'lumotlari topilmadi.");
...
🔄 <b>Oxirgi yangilanish:</b> ${stats.updatedAt.toLocaleString("uz-UZ", {
      ...
      `;
            ...
            await ctx.reply("❌ Batafsil statistikani ko'rsatishda xatolik yuz berdi.");
        }
    }
    // ==================== KEYBOARD CREATION ====================
    // Buyurtmalar uchun tugmalar yaratish (pagination + buyurtma ID lar)
    function createOrdersKeyboard(currentPage, totalPages, orders) {
        const keyboard = [];
        // Buyurtma tugmalari
        orders.forEach((order) => {
            keyboard.push([
                telegraf_1.Markup.button.callback(`${order.orderNumber} - ${order.customerName}`, `order_${order.id}`),
            ]);
        });
        ...
        if (totalPages > 1) {
            const paginationRow = [];
            if (currentPage > 1) {
                paginationRow.push(telegraf_1.Markup.button.callback("⬅️ Oldingi", `page_${currentPage - 1}`));
            }
            if (currentPage < totalPages) {
                paginationRow.push(telegraf_1.Markup.button.callback("Keyingi ➡️", `page_${currentPage + 1}`));
            }
        ...
        // Boshqa tugmalar
        keyboard.push([
            telegraf_1.Markup.button.callback("🔄 Yangilash", "refresh_orders"),
            telegraf_1.Markup.button.callback("🔙 Bosh menyu", "admin_main"),
        ]);
        return telegraf_1.Markup.inlineKeyboard(keyboard);
    }
    // Buyurtma ma'lumotlari uchun tugmalar
    function createOrderDetailsKeyboard(orderId, currentStatus) {
        const keyboard = [];
        // Holat yangilash tugmalari
        if (currentStatus !== "CONFIRMED") {
            keyboard.push([
                telegraf_1.Markup.button.callback("✅ Tasdiqlash", `status_${orderId}_CONFIRMED`),
            ]);
        }
        if (currentStatus !== "COMPLETED") {
            keyboard.push([
                telegraf_1.Markup.button.callback("🎯 Bajarildi", `status_${orderId}_COMPLETED`),
            ]);
        }
        if (currentStatus !== "CANCELLED") {
            keyboard.push([
                telegraf_1.Markup.button.callback("❌ Bekor qilish", `status_${orderId}_CANCELLED`),
            ]);
        }
        // Navigatsiya tugmalari
        keyboard.push([
            telegraf_1.Markup.button.callback("🔙 Buyurtmalarga qaytish", "back_to_orders"),
            telegraf_1.Markup.button.callback("🏠 Bosh menyu", "admin_main"),
        ]);
        return telegraf_1.Markup.inlineKeyboard(keyboard);
    }
