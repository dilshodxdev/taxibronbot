...
exports.orderTaxiScene = new telegraf_1.Scenes.WizardScene("ORDER_TAXI_SCENE", 
...
async (ctx) => {
    ...
    if (ctx.callbackQuery && "data" in ctx.callbackQuery) {
        ...
        if (routeData === "route_tashkent_to_xorazm") {
            ...
            const districtMessage = "🏙️ Toshkent viloyatidan qaysi tumanni tanlaysiz?";
            await ctx.reply(districtMessage, {
                reply_markup: (0, tashkentDistricts_1.tashkentDistrictsKeyboard)().reply_markup,
            });
        }
            ...
            const districtMessage = "🏘️ Xorazm viloyatidan qaysi tumanni tanlaysiz?";
            await ctx.reply(districtMessage, {
                reply_markup: (0, xorazmDistricts_1.xorazmDistrictsKeyboard)().reply_markup,
            });
        }
            ...
            await ctx.reply(regionMessage, {
                reply_markup: (0, fromRegions_1.fromRegionsKeyboard)().reply_markup,
            });
        }
    ...
    const directionKeyboard = telegraf_1.Markup.inlineKeyboard([
        [
            telegraf_1.Markup.button.callback("🚖 Xorazm → Toshkent", "route_xorazm_to_tashkent"),
            telegraf_1.Markup.button.callback("🚖 Toshkent → Xorazm", "route_tashkent_to_xorazm"),
        ],
        [telegraf_1.Markup.button.callback("🌍 Boshqa yo'nalish", "route_other")],
    ]);
...
async (ctx) => {
    ...
    if (ctx.scene.session.direction === "tashkent_to_xorazm") {
        ...
        if (callbackData.startsWith("tashkent_")) {
            ...
            // Xorazm tumanlarini ko'rsatish
            const toDistrictMessage = "🏘️ Xorazm viloyatidan qaysi tumanni tanlaysiz?";
            const toDistrictKeyboard = createDistrictKeyboard(districtMap_1.XORAZM_DISTRICTS, "xorazm");
        ...
        if (callbackData.startsWith("xorazm_")) {
            ...
            ctx.scene.session.fromDistrict = district;
            // Toshkent tumanlarini ko'rsatish
            const toDistrictMessage = "🏙️ Toshkent viloyatidan qaysi tumanni tanlaysiz?";
            const toDistrictKeyboard = createDistrictKeyboard(districtMap_1.TASHKENT_DISTRICTS, "tashkent");
            await ctx.reply(toDistrictMessage, {
                reply_markup: toDistrictKeyboard.reply_markup,
            });
        }
...
async (ctx) => {
    ...
    if (!fullName || fullName.length < 2) {
        await ctx.reply("❌ Ism juda qisqa! Iltimos, to'liq ismingizni kiriting:");
        return ctx.wizard.selectStep(3);
    }
    ctx.scene.session.fullName = fullName;
    // Telefon raqam so'rash - text yoki contact button bilan
    const phoneMessage = "📞 Iltimos, telefon raqamingizni kiriting (+998901234567):";
    // Contact button reply keyboard da ishlatiladi, inline keyboard da emas
    const phoneKeyboard = telegraf_1.Markup.keyboard([
        [telegraf_1.Markup.button.contactRequest("📱 Kontakt yuborish")],
    ]).resize();
...
async (ctx) => {
    ...
    if (!isValidPhone(phone)) {
        await ctx.reply("❌ Noto'g'ri telefon raqam! Iltimos, to'g'ri formatda kiriting (+998901234567):");
...
function createDistrictKeyboard(districts, prefix) {
    const buttons = [];
    // Har 2 ta tuman bir qatorda
    for (let i = 0; i < districts.length; i += 2) {
        const row = [
            telegraf_1.Markup.button.callback(districts[i], `${prefix}_${districts[i]}`),
        ];
        if (i + 1 < districts.length) {
            row.push(telegraf_1.Markup.button.callback(districts[i + 1], `${prefix}_${districts[i + 1]}`));
        }
        buttons.push(row);
    }
    // "Boshqa tuman" tugmasi
    buttons.push([telegraf_1.Markup.button.callback("Boshqa tuman", `${prefix}_other`)]);
    return telegraf_1.Markup.inlineKeyboard(buttons);
}
// Viloyat tugmalarini yaratish
function createRegionKeyboard(prefix) {
    const buttons = [];
    // Har 2 ta viloyat bir qatorda
    for (let i = 0; i < districtMap_1.REGIONS.length; i += 2) {
        const row = [telegraf_1.Markup.button.callback(districtMap_1.REGIONS[i], `${prefix}_${districtMap_1.REGIONS[i]}`)];
        if (i + 1 < districtMap_1.REGIONS.length) {
            row.push(telegraf_1.Markup.button.callback(districtMap_1.REGIONS[i + 1], `${prefix}_${districtMap_1.REGIONS[i + 1]}`));
        }
        buttons.push(row);
    }
    return telegraf_1.Markup.inlineKeyboard(buttons);
...
async function showOrderSummary(ctx) {
    ...
    if (!session.fromRegion ||
        ...
        await ctx.reply("❌ Buyurtma ma'lumotlari to'liq emas. Iltimos, qaytadan urinib ko'ring...", {
...
📍 <b>Yo'nalish:</b> ${session.fromRegion}${session.fromDistrict ? ` - ${session.fromDistrict}` : ""} → ${session.toRegion}${session.toDistrict ? ` - ${session.toDistrict}` : ""}
...
✅ Buyurtmangiz qabul qilindi! Tez orada siz bilan bog'lanishadi.
  `;
    ...
    const adminId = config_1.CONFIG.ADMIN_ID;
    if (adminId) {
