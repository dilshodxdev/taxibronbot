...
export class DatabaseService {
  ...
  async upsertUser(telegramId: string, userData: {
    username?: string;
    firstName?: string;
    lastName?: string;
    fullName: string;
  ...
  async getOrders(page: number = 1, limit: number = 5): Promise<{
    orders: any[];
    total: number;
    totalPages: number;
    currentPage: number;
  ...
  async getOrdersByStatus(status: OrderStatus): Promise<number> {
    return await this.prisma.order.count({
      where: { status }
    });
  }
  ...
  async getOrdersByUserId(telegramId: string): Promise<any[]> {
  ...
  async upsertUserSession(userId: number, sessionData: {
    step?: string;
    direction?: string;
  ...
  async getBotStatistics(): Promise<any | null> {
