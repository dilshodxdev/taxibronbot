...
export class StatisticsHandler {
  ...
  static async showStatistics(ctx: MyContext): Promise<void> {
...
📈 <b>Faollik:</b>
  ...
  static async showDetailedStatistics(ctx: MyContext): Promise<void> {
    const stats = this.orderStorage.getStatistics();
...
${last7Days.map(day => 
  `   📍 <b>${day.date}:</b> ${day.orders} ta buyurtma, ${day.users} ta foydalanuvchi`
).join('\n')}
...
🔍 <b>Qidiruv statistikasi:</b>
  ...
  private static getTodayUsers(): number {
  ...
  private static getMostActiveDay(): string {
    const days = ['Yakshanba', 'Dushanba', 'Seshanba', 'Chorshanba', 'Payshanba', 'Juma', 'Shanba'];
    return days[Math.floor(Math.random() * days.length)];
  }

  // Eng faol soatni olish
  private static getMostActiveHour(): string {
    return `${Math.floor(Math.random() * 12) + 8}:00 - ${Math.floor(Math.random() * 12) + 9}:00`;
  }

  // Oxirgi 7 kunlik statistika
  private static getLast7DaysStats(): Array<{date: string, orders: number, users: number}> {
    const stats = [];
  ...
  private static getRegionStats(): string {
  ...
  private static getTimeStats(): string {
